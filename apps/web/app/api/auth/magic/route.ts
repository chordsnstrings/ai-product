import { z } from 'zod';
import { requestMagicLink } from '@arkiv/auth';
...
export const POST = route(async (req) => {
  const input = await body(req, z.object({ email: z.string().max(254), next: z.string().max(300).nullish(), purpose: z.enum(['login', 'claim', 'resume']).default('login') }));

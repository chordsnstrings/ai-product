  ...
  return (
    <>
      <Table head={['Project', 'Kind', 'State', 'Updated', 'Provider jobs', 'Cost', '']} rows={d0.projects.map((p) => {
        ...
        return [<Mono key="i">{String(p.id).slice(0, 8)}</Mono>, p.kind as string, <span key="s" style={{ color: stuck ? 'var(--risk)' : undefined }}>{String(p.state).toLowerCase()}{stuck ? ' (stuck)' : ''}{p.failure_reason ? ` — ${p.failure_reason}` : ''}</span>, ago(p.updated_at), p.jobs as number, money(p.cost), canRetry && (['PROVIDER_FAILED', 'NEEDS_USER_ACTION'].includes(p.state as string) || stuck) && p.state !== 'STORYBOARD_APPROVED' ? <ActButton key="r" small action="project.retry" payload={{ projectId: p.id, workspaceId: id }} reason="Retry reason (a stalled run resumes without a new reservation)">{stuck ? 'Resume' : 'Retry'}</ActButton> : null];
      })} />

...
export default async function Prompts({ searchParams }: { searchParams: Promise<{ run?: string }> }) {
  ...
  const d0 = await withAdmin(async (tx) => ({
    routes: await tx`select task, prompt_version, rollout_pct from model_routes order by task`,
  ...
  return (
    <Page title="Prompt registry & evaluations">
      ...
      <Section title="Eval runs">
        <Table head={['When', 'Dataset', 'Status', 'Score', 'Cases', 'By', '']} rows={d0.runs.map((r) => [dt(r.created_at), r.dataset as string, <span key="s" className={`ak-chip ${r.status === 'passed' ? 'ak-chip--ok' : r.status === 'failed' ? 'ak-chip--risk' : ''}`}>{r.status as string}</span>, r.score === null ? '—' : pct(Number(r.score)), r.cases as number, r.name as string, <a key="v" href={`/prompts?run=${r.id}`}>view</a>])} empty="No runs yet." />
      ...
      <Section title="Live routes">
        <Table head={['Task', 'Prompt version', 'Rollout']} rows={d0.routes.map((r) => [<Mono key="t">{r.task as string}</Mono>, <Mono key="p">{r.prompt_version as string}</Mono>, `${r.rollout_pct}%`])} />
      </Section>

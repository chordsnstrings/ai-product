...
export default async function Integrations({ searchParams }: { searchParams: Promise<{ stale?: string }> }) {
  ...
  const d0 = await withAdmin(async (tx) => {
    ...
    return {
      ...
      stale: await tx`select i.id, i.workspace_id, w.name, i.provider, i.status, i.last_success_at, i.token_expires_at, i.scopes, i.error from integrations i join workspaces w on w.id = i.workspace_id
                      where i.status <> 'disconnected' and (i.status <> 'active' or i.last_success_at is null or i.last_success_at < now() - interval '7 days') ${notTest(tx, prefs, 'i.workspace_id')} order by i.last_success_at nulls first limit 100`,
  ...
  return (
    <Page title="Integrations health" sub={`${d0.shops[0]!.n} Shopify shops routed (one workspace per shop). Test accounts ${prefs.includeTest ? 'included' : 'excluded'}.`} actions={<ActButton action="integration.verify" payload={{}}>Verify Shopify webhooks</ActButton>}>

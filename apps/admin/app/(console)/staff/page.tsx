  ...
  return (
    <Page title="Staff" sub={`Roles: ${StaffRole.join(', ')}`}>
      <Table head={['Name', 'Email', 'Roles', 'Second factor', 'Active', 'Last seen', 'Actions (30d)', 'Roles confirmed', '']} rows={d0.rows.map((x) => [
        ...
        x.active ? 'yes' : 'no', ago(x.last_seen), <a key="t" href={`/audit?staff=${encodeURIComponent(x.email as string)}`}>{x.actions as number}</a>,
      ...
      <Section title="Invite staff">
        <div className="ak-panel" style={{ maxWidth: 520 }}>
          <p className="ak-small ak-muted">The account is created without roles; the roles you request take effect once another SUPER_ADMIN approves them (four-eyes). They sign in with the authenticator secret, then add a passkey under Passkeys.</p>

...
/**
 * Plan 05 §3. Staff can lock, force logout, resend verification and send a sign-in link (sign-in is passwordless,
 * so that is the reset) — never set a credential. Sessions show the edge's location. SUPPORT sees PII
...
export default async function User({ params }: { params: Promise<{ id: string }> }) {
  ...
  const d0 = await withAdmin(async (tx) => {
    ...
    return {
      ...
      sessions: await tx`select id, ip, user_agent, geo, created_at, last_seen_at, revoked_at from sessions where user_id = ${id} order by last_seen_at desc limit 30`,
  ...
  return (
    <Page title={pii.email(u.email)} sub={<><Mono>{id}</Mono> · {u.locked_at ? `locked: ${u.locked_reason}` : 'active'} · verified {dt(u.email_verified_at)}{pii.mask ? ' · PII masked for your role' : ''}</>} actions={
      <>
        ...
        {!u.locked_at ? <ActButton action="user.send_login_link" payload={{ userId: id }} reason="Why (ticket #) — sign-in is passwordless, so this is the reset">Send sign-in link</ActButton> : null}
      ...
      <Section title="Sessions"><Table head={['Created', 'Last seen', 'IP', 'Location', 'Device', 'Status']} rows={d0.sessions.map((x) => [dt(x.created_at), dt(x.last_seen_at), <Mono key="i">{pii.ip(x.ip)}</Mono>, geoLabel(x.geo as EdgeGeo | null) || '—', <span key="d" className="ak-small">{pii.mask ? pii.ua(x.user_agent) : String(x.user_agent ?? '').slice(0, 70)}</span>, x.revoked_at ? 'revoked' : 'active'])} /></Section>

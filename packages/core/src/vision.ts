...
export interface MediaReviewFlags {
  ...
  /**
   * What raised the flag: the product analyst looking at the photo, the file's name/source, the merchant's declaration
   * on an import (`declared`), or a merchant-attested before/after upload (`merchant`).
   */
  sources: ('vision' | 'name' | 'declared' | 'merchant')[];
}

...
export async function versionCreative(tx: Tx, ctx: Pick<TenantContext, 'workspaceId' | 'actor'>, v: CreativeVersion): Promise<string> {
  ...
  const [cr] = await tx`insert into creatives (workspace_id, sku_id, origin, parent_creative_id, project_id, genome, genome_version, final_asset_ids, composition, ai_generated, synthetic_people, visual_fingerprint_id)
                        values (${ctx.workspaceId}, ${v.skuId}, 'generated', ${v.parentCreativeId}, ${v.projectId}, ${tx.json(v.genome as never)}, (${GENOME_VERSION_SQL(tx)}),
                                ${v.finalAssetIds}, ${tx.json((v.composition ?? null) as never)}, ${!!parent?.ai_generated}, ${!!parent?.synthetic_people}, ${(parent?.visual_fingerprint_id as string | null | undefined) ?? null})
                        returning id`;

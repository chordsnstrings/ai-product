  ...
  try {
    ...
    await withTempDir(async (dir) => {
      ...
      await withTenant(ws, async (tx) => {
        ...
        const [cr] = await tx`
          insert into creatives (workspace_id, sku_id, origin, project_id, genome, genome_version, final_asset_ids, composition, ai_generated, synthetic_people, visual_fingerprint_id)
          values (${ws}, ${sku.id}, 'generated', ${projectId}, ${tx.json(genome as never)},
            ...
            ${(sb.visual_fingerprint_id as string | null | undefined) ?? (fp?.id as string | null | undefined) ?? null})
          returning id`;

...
export interface ScoringContext {
  themes: { label: string; prevalence: number }[];
  angleTests: Map<string, number>;
  learnings: { angle: string; state: string; positive: boolean }[];
...
export function scoreProposal(p: Proposal, s: ScoringContext): Scored {
  ...
  return { proposal: p, slot: slotFor(p, s), score: gates.passed ? score : 0, breakdown: b, gates };
}
...
async function scoringContext(tx: Tx, skuId: string): Promise<ScoringContext & { maturity: SkuMaturity; basis: 'performance' | 'context_limited' | 'cold_start' }> {
  ...
  return {
    themes: themes.map((t) => ({ label: t.label as string, prevalence: Number(t.prevalence) })),
    angleTests: new Map(tests.map((t) => [t.angle as string, t.n as number])),
    learnings: learnings.map((l) => ({ angle: l.angle as string, state: l.state as string, positive: true })),
    ...
    basis: !integ!.n ? (hasPerf ? 'context_limited' : 'cold_start') : integ!.fresh < integ!.n ? 'context_limited' : hasPerf ? 'performance' : 'cold_start',
  };
}

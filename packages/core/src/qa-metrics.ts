 ...
 * events are tied to their project through the storyboard. Test workspaces are excluded. Admin/system scope:
 * reads across tenants.
 */
export async function repeatedFidelityFailures(tx: Tx, days: number): Promise<{ paid: number; repeated: number; rate: number }> {
  const [r] = await tx`
    with paid as (
      ...
      where a.purpose in ('taste','standalone','creative_test') and a.idempotency_key like 'produce:%'
        ...
        and a.workspace_id not in (select id from workspaces where is_test)
    ),
    repeated as (

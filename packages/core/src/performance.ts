 ...
 */
export async function stashPendingConnection(tx: Tx, ctx: TenantContext, provider: 'meta' | 'tiktok', token: string, accounts: PendingAccount[]) {
  assertCan(ctx, 'integration.manage');
  await tx`delete from pending_connections where expires_at < now()`;
  const [r] = await tx`insert into pending_connections (workspace_id, provider, token_enc, accounts, created_by)
                       values (${ctx.workspaceId}, ${provider}, ${encryptToken(token)}, ${tx.json(accounts as never)}, ${ctx.actor.kind === 'user' ? ctx.actor.id : null})
                       returning id`;
...
export async function connectSelectedAccounts(tx: Tx, ctx: TenantContext, pendingId: string, accountIds: string[], opts: { replace?: boolean } = {}) {
  assertCan(ctx, 'integration.manage');
  const [p] = await tx`select id, provider, token_enc, accounts from pending_connections where id = ${pendingId} and expires_at > now() for update`;
  ...
  for (const a of chosen) {
    ids.push(await saveIntegration(tx, ctx, { provider, externalAccountId: a.id, displayName: a.name, token, scopes: ['ads_read'], currency: a.currency, timezone: a.timezone }));
  }
  let disconnected = 0;
  if (opts.replace) {
 ...
 */
export function parsePerformanceCsv(csv: string, opts: { timezone?: string | null } = {}): NormalizedObservation[] {

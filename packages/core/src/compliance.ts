 ...
 */
export const RULES_VERSION = 'claims-rules@1.0.0';
...
export const RULES: Rule[] = [
  ...
  { id: 'structure.function', pattern: /\b(boosts?|stimulates?|increases?|repairs?|regenerates?|rebuilds?)\s+(collagen|elastin|cell(s|ular)?|dna|hormones?|blood flow|skin cells)\b|\bcell(ular)?\s+(renewal|regeneration|turnover)\b|\bpenetrates?\s+(the\s+)?dermis\b/i, risk: 'prohibited', status: 'BLOCKED', category: 'structure_function', reason: 'Claims that change the structure or function of the body make a product a drug (FDA).', alternative: 'Use "the look of firmer skin" style appearance claims.' },
  { id: 'sun.protection', pattern: /\b(spf\s*\d*|sun\s*(screen|block|protection)|uv\s*(protection|defen[cs]e)|protects?\s+(against|from)\s+(the\s+)?sun)\b/i, risk: 'prohibited', status: 'BLOCKED', category: 'otc_drug', reason: 'Sun protection is an OTC drug claim; SPF products are out of V1 scope.' },

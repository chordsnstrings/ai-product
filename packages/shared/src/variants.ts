...
/**
 * Deterministic experiment assignment (Offer Engine, landing variants), with no dependency on the database, so the
 * web proxy can assign a landing variant before a static page is served. Server-only (node:crypto).
 */
...
/**
 * Sticky per subject. The hash maps to a uniform point in [0, total) and is compared against cumulative weights, so
 * fractional weights (0.5/0.5, 0.2/0.8) split traffic in proportion rather than collapsing onto one integer bucket.
 */

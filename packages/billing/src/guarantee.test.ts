...
async function failedPaidTaste() {
  ...
  const [pu] = await ownerPool()`insert into purchases (workspace_id, kind, project_id, amount_micros, stripe_payment_intent_id, status, created_by, paid_at)
                                 values (${t.workspaceId}, 'taste', ${projectId}, 19000000, 'pi_guarantee', 'paid', 'test', now()) returning id`;
  await withTenant(t.workspaceId, async (tx) => {
    await append(tx, ctx, { type: 'CREDIT_GRANTED', unit: 'taste', amount: 1, idempotencyKey: 'pay:guarantee' });
...
describe('quality guarantee refund', () => {
  it('failing the production queues the refund; the refund returns the money once and never also the credit', async () => {
    ...
    expect(gw.refunds).toEqual([{ pi: 'pi_guarantee', amount: undefined, idempotencyKey: `guarantee:${purchaseId}` }]);
    await withTenant(t.workspaceId, async (tx) => {
